...
mod tests {
    ...
    fn test_fdt_discovery() {
        ...
        #[rustfmt::skip]
        const MOCK_DTB: &[u8] = &[
            0xd0, 0x0d, 0xfe, 0xed, 0x00, 0x00, 0x00, 0xA0, // magic, totalsize (updated)
            0x00, 0x00, 0x00, 0x38, 0x00, 0x00, 0x00, 0x80, // off_dt_struct, off_dt_strings (updated)
            ...
            0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x48, // size_dt_strings (updated), size_dt_struct (updated)
            // rsvmap
            ...
            // #address-cells = 1
            0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, // PROP, len=4, name=0 (#address-cells)
            0x00, 0x00, 0x00, 0x01,

            // #size-cells = 1
            0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0F, // PROP, len=4, name=15 (#size-cells)
            0x00, 0x00, 0x00, 0x01,

            0x00, 0x00, 0x00, 0x01, 0x74, 0x65, 0x73, 0x74, 0x40, 0x31, 0x32, 0x33, 0x34, 0x00, 0x00, 0x00, // BEGIN_NODE "test@1234"
            0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x1B, // PROP, len=9, name=27 (compatible)
            0x74, 0x65, 0x73, 0x74, 0x2d, 0x64, 0x65, 0x76, 0x00, 0x00, 0x00, 0x00, // "test-dev\0" + padding
            0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x26, // PROP, len=8, name=38 (reg)
            0x00, 0x00, 0x12, 0x34, 0x00, 0x00, 0x10, 0x00,
            0x00, 0x00, 0x00, 0x02, // END_NODE
            0x00, 0x00, 0x00, 0x02, // END_NODE
            0x00, 0x00, 0x00, 0x09, // FDT_END
            // strings
            0x23, 0x61, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x2d, 0x63, 0x65, 0x6c, 0x6c, 0x73, 0x00, // "#address-cells\0" (0)
            0x23, 0x73, 0x69, 0x7a, 0x65, 0x2d, 0x63, 0x65, 0x6c, 0x6c, 0x73, 0x00, // "#size-cells\0" (15)
            0x63, 0x6f, 0x6d, 0x70, 0x61, 0x74, 0x69, 0x62, 0x6c, 0x65, 0x00, // "compatible\0" (27)
            0x72, 0x65, 0x67, 0x00, // "reg\0" (38)
        ];

        let fdt = Fdt::new(MOCK_DTB).expect("Invalid mock DTB");
        let node = find_node_by_compatible(&fdt, "test-dev").expect("Node not found"); // [FD7]
        assert_eq!(node.name, "test@1234");

        let (addr, size) = get_node_reg(&node).expect("Reg not found"); // [FD8]
