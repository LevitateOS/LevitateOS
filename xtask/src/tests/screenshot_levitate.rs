...
fn qmp_socket(arch: &str) -> String {
    format!("./levitate-test-{arch}.sock")
}
...
pub fn run() -> Result<()> {
    ...
    match x86_64_result {
        ...
        Err(e) => println!("  ⚠️  x86_64: {e} (expected)"),
    }
...
fn run_arch(arch: &str) -> Result<()> {
    ...
    // Wait for OS to boot
    println!("[{arch}] ⏳ Waiting for boot ({BOOT_TIMEOUT_SECS} seconds)...");
    std::thread::sleep(Duration::from_secs(BOOT_TIMEOUT_SECS));

    let mut client = QmpClient::connect(&socket)?;

    // Take screenshot
    let screenshot = format!("tests/screenshots/levitate_{arch}.ppm");
    ...
    println!("[{arch}] ✅ Screenshot captured!");
    Ok(())
}
...
fn start_levitate(arch: &str) -> Result<std::process::Child> {
    ...
    let mut builder = QemuBuilder::new(arch_enum, profile).enable_qmp(&socket);
